...
export class ConnectWiseAuthController {
  ...
  async connectWiseAuth(
    ...
    try {
      ...
      const authUrl =
        `${authRequest.serverUrl}/v4_6_release/apis/3.0/system/oauth/authorize?` +
      ...
      response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
        ...
        error:
          err instanceof Error
            ? err.message
            : 'Failed to initiate ConnectWise OAuth flow',
      });
    }
  }
    ...
    try {
      ...
      if (error) {
        return response.redirect(
          `${process.env.FRONTEND_URL}/integrations?error=${encodeURIComponent(`ConnectWise OAuth error: ${error}`)}`,
        );
      }
      ...
      if (!storedState) {
        return response.redirect(
          `${process.env.FRONTEND_URL}/integrations?error=${encodeURIComponent('Invalid state parameter')}`,
        );
      }
      ...
      response.redirect(
        `${process.env.FRONTEND_URL}/integrations?success=true&provider=connectwise`,
      );
    } catch (_err) {
      response.redirect(
        `${process.env.FRONTEND_URL}/integrations?error=${encodeURIComponent('Failed to complete ConnectWise OAuth flow')}`,
      );
    }
  }
  ...
  async disconnectConnectWise(
    ...
    try {
      ...
      const integration =
        await this.integrationService.getIntegrationByUserIdAndProvider(
          userId,
          'connectwise',
        );
